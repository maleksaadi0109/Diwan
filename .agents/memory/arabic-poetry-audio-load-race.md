...
## Desktop playback format
For Tauri/WebKit2GTK, do not rely on Blob-backed MP3 for playback. Use a PCM WAV playback copy with deterministic byte offsets; keep MP3 for web/export and keep the 16 kHz mono processing WAV separate for VAD/ASR.

**Why:** The problem was actual audible playback-head movement in the desktop engine, not merely a bogus timestamp shown by the UI. A timestamp filter cannot stop the native decoder from jumping within an MP3.

**How to apply:** Prepare/cache a playback-quality PCM WAV for absolute local MP3 paths before creating the desktop Blob URL. During any asynchronous source preparation, immediately detach the previous track so old audio cannot continue under a newly loaded poem.
