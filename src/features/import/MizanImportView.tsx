  ...
  const handleImportPoem = async () => {
    ...
    try {
      ...
      try {
        ...
        // Update verses with explanations in SQLite
        for (const v of newPoem.verses) {
          const exps = explanationsMap.get(v.externalId || "");
          if (exps && exps.length > 0) {
            await repo.saveVerseExplanations(v.id, exps);
          }
        }
      } catch (err_exp) {
